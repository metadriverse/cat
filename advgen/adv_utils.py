...
import advgen.globals, advgen.structs, advgen.utils, advgen.utils_cython
...
Normalizer = advgen.utils.Normalizer
...
def get_instance(args: advgen.utils.Args, inputs, decoded_example, file_name='tmp',
    ...
    if True:
        ...
        if args.influencer_pred_file_path is not None:
            if globals.influencer_pred is None:
                ...
                globals.influencer_pred = advgen.structs.load(args.influencer_pred_file_path)
        ...
        if args.relation_file_path is not None:
            if args.do_test:
                ...
                if globals.interactive_relations is None:
                    ...
                    globals.interactive_relations = advgen.structs.load(args.relation_file_path)
        ...
        if 'direct_relation_label' in args.other_params:
            ...
            if globals.direct_relation is None:
                ...
                globals.direct_relation = advgen.structs.load(args.direct_relation_path)
        ...
        if 'gt_relation_label' in args.other_params:
            ...
            if globals.relation_pred is None:
                ...
                globals.relation_pred = advgen.structs.load(args.relation_pred_file_path)
        ...
        # training reactor following Influencer->Reactor
            ...
            if 'pred_all_agents' in args.other_params:
                ...
                if globals.all_relevant_agent_ids is None:
                    ...
                    globals.all_relevant_agent_ids = advgen.structs.load(args.all_agent_ids_path)
        ...
        speed = advgen.utils.get_dis_point2point((gt_trajectory[0, history_frame_num - 1, 5], gt_trajectory[0, history_frame_num - 1, 6]))
        waymo_yaw = gt_trajectory[0, last_valid_index, 4]
        track_type_int = tracks_type[0]
        trajectory_type = advgen.utils_cython.classify_track(gt_future_is_valid[0], gt_trajectory[0])
        headings = gt_trajectory[0, history_frame_num:, 4].copy()

        angle = -waymo_yaw + math.radians(90)

        normalizer = advgen.utils.Normalizer(gt_trajectory[0, last_valid_index, 0], gt_trajectory[0, last_valid_index, 1], angle)

        gt_trajectory[:, :, :2] = advgen.utils_cython.get_normalized(gt_trajectory[:, :, :2], normalizer)
        if 'train_reactor' in args.other_params:
            if 'gt_influencer_traj' in args.other_params:
                gt_influencer[:, :2] = advgen.utils_cython.get_normalized(gt_influencer[:, :2][np.newaxis, :], normalizer)[0]
            else:
                influencer_pred_rst[:, :, :2] = advgen.utils_cython.get_normalized(influencer_pred_rst[:, :, :2], normalizer)
        if 'relation_wpred' in args.other_params:
            gt_trajectory_pair[0, :, :, :2] = advgen.utils_cython.get_normalized(gt_trajectory_pair[0, :, :, :2], normalizer)
            gt_trajectory_pair[1, :, :, :2] = advgen.utils_cython.get_normalized(gt_trajectory_pair[1, :, :, :2], normalizer)
    ...
    if 'train_reactor' in args.other_params and 'raster_inf' in args.other_params:
        if 'gt_influencer_traj' in args.other_params:
            vectors, polyline_spans, trajs = advgen.utils_cython.get_agents(gt_trajectory, gt_future_is_valid, tracks_type,
                                                                     args.visualize, args,
                                                                     gt_influencer)
        else:
            vectors, polyline_spans, trajs = advgen.utils_cython.get_agents(gt_trajectory, gt_future_is_valid, tracks_type,
        ...
        vectors, polyline_spans, trajs = advgen.utils_cython.get_agents(gt_trajectory, gt_future_is_valid, tracks_type, args.visualize, args)

    map_start_polyline_idx = len(polyline_spans)

    vectors_, polyline_spans_, goals_2D, lanes = advgen.utils_cython.get_roads(decoded_example, normalizer, args)
    ...
    stage_one_label = np.argmin([advgen.utils.get_dis(lane, gt_trajectory[0, -1, :2]).min() for lane in lanes]) if len(lanes) > 0 else 0
    ...
    mapping['goals_2D_labels'] = np.argmin(advgen.utils.get_dis(goals_2D, labels[final_idx]))

    # Add subgoal info.
    if args.classify_sub_goals:
        if labels.shape[0] > 29:
            mapping['goals_2D_labels_3s'] = np.argmin(advgen.utils.get_dis(goals_2D, labels[29]))
        if labels.shape[0] > 49:
            mapping['goals_2D_labels_5s'] = np.argmin(advgen.utils.get_dis(goals_2D, labels[49]))

    # Add traffic state info.
    if 'tf_poly' in args.other_params:
        traffic_light_vectors = advgen.utils_cython.get_traffic_lights(decoded_example, normalizer, args)
...
def _parse(decoded_example):
    ...
    '''
    input_states (128, 11, 2)
    gt_future_states (128, 91, 7)
    gt_future_is_valid (128, 91)
    object_type (128,)
    tracks_to_predict (128,)
    interactive_tracks_to_predict (128,)
    sample_is_valid (128,)
    '''
    return inputs, decoded_example

def process_data(raw_data, args: advgen.utils.Args, trajectory_type_2_ex_list=None, balance_queue=None):
    ex_list = []
    dataset = [raw_data]
    for step, data in enumerate(dataset):
        ...
        if args.do_eval:
            ...
            if 'joint_eval' in args.other_params:
                ...
                if types_are_ok((tracks_type[0], tracks_type[1]), args) or types_are_ok((tracks_type[1], tracks_type[0]), args):
                    for select in range(predict_agent_num):
                        t = get_instance(args, inputs, decoded_example, 'tmp',select=select)
            ...
            if 'train_pair_interest' in args.other_params:
                instance = [get_instance(args, inputs, decoded_example, 'tmp',select=select)
